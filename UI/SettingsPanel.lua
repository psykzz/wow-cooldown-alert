...
-- Uses the modern Settings API on retail (Dragonflight+); slash commands
-- work on all versions. Currently disabled in CooldownAlert.toc (commented
-- out) pending further verification -- re-enable that line to turn it on.

local ADDON_NAME = CooldownAlert.ADDON_NAME
...
-- ── Retail Settings API (Dragonflight / The War Within / Midnight) ───────────
...
local function InitRetailSettings()
    ...
    -- Changes are applied immediately and ApplySettings is called.
    local function Proxy(varKey, varType, name)
        return Settings.RegisterProxySetting(
            ...
            function() return CooldownAlertDB[varKey] end,
            function(_, v)
                if v == nil then return end
                CooldownAlertDB[varKey] = v
                CooldownAlert.PreviewFrame.ResetTimer()
    ...
    local holdTimeSetting = Proxy("holdTime", Settings.VarType.Number, "Hold Time")
    ...
    local fadeOutSetting = Proxy("fadeOutTime", Settings.VarType.Number, "Fade Out Time")
    ...
    local fontSizeSetting = Settings.RegisterProxySetting(
        category,
        "CooldownAlert_fontSize",
        Settings.VarType.Number,
        "Font Size",
        CooldownAlertDB_Defaults.fontSize,
    ...
    -- Font Face (dropdown)
    local fontFaceSetting = Proxy("fontFace", Settings.VarType.String, "Font")
    ...
    -- Font Style (dropdown)
    local fontFlagsSetting = Proxy("fontFlags", Settings.VarType.String, "Font Style")
    ...
    -- Text Format (dropdown) - only meaningful when TextDisplay is active
    -- (the CooldownDisplay path doesn't render text).
    if not CooldownAlert.SupportsCooldownDurationObjects() then
        local textFormatSetting = Proxy("textFormat", Settings.VarType.String, "Text Format")
    ...
    -- ── Preview frame visibility ─────────────────────────────────────────────
    -- Shown (at the configured screen position) only while the Cooldown
    -- Alert settings category is the active one in the Settings panel.
    CooldownAlert.PreviewFrame.Create()
    ...
    local function RefreshPreviewVisibility()
        ...
        if isOurCategory then
            ...
            CooldownAlert.PreviewFrame.Show()
        else
            CooldownAlert.PreviewFrame.Hide()
        end
    end
...
settingsFrame:SetScript("OnEvent", function(self, event, addonName)
    ...
    -- Core/Bootstrap.lua's ADDON_LOADED handler runs first (registered
    -- earlier), so CooldownAlertDB is already initialised here.
