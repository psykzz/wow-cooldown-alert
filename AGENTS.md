...
- Prefer the duration-object cooldown APIs (`CooldownAlert.SupportsCooldownDurationObjects()` in `Core/ClientCapabilities.lua`) when *rendering* a cooldown -- they sidestep secret-value arithmetic entirely and must be used over raw `(startTime, duration)` math whenever supported.
  - Exception: decisions that need a numeric threshold (e.g. "is this cooldown long enough to be worth alerting on?", see `CooldownAlert.ShouldShowAlert` in `Core/ApiWrappers.lua`) have no duration-object equivalent -- a duration object is opaque and exposes no readable magnitude, by design. These use the raw `(startTime, duration, isSecret)` wrappers on every client, but only after checking `isSecret` on both values first, and always resolve to "show" rather than guessing when a value is secret.
